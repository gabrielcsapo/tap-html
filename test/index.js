...
test("tap-html", (t) => {
  ...
  t.test("should be able to parse test that starts with range", (t) => {
    const testWithPlanAsRange = fs.createReadStream(
      path.resolve(__dirname, "./fixtures/test-with-plan-as-range.txt")
    );
    testWithPlanAsRange.pipe(
      parser((res) => {
        t.equal(res.ok, true);
        t.equal(res.count, 2);
        t.equal(res.pass, 2);
        ...
        t.equal(res.todo, 0);
        t.equal(res.skip, 0);
        console.dir(res.plan);
        t.equal(res.plan.start, 1);
        t.equal(res.plan.end, 2);
        t.equal(res.plan.skipAll, false);
        t.equal(res.plan.skipReason, "");
        t.equal(res.plan.comment, "");
        ...
        t.equal(
          res.tests[1].assertions[0].name,
          "And this one is another test description"
        );
  ...
  t.test("should be able to parse flat test", (t) => {
    const flat = fs.createReadStream(
      path.resolve(__dirname, "./fixtures/flat.txt")
    );
    flat.pipe(
      parser((res) => {
        t.equal(res.ok, true);
        ...
        t.equal(res.tests[0].assertions[0].type, "assert");
        t.equal(res.tests[0].assertions[0].number, 1);
        t.equal(res.tests[0].assertions[0].diag, null);
        ...
        t.equal(res.tests[1].assertions[0].type, "assert");
        t.equal(res.tests[1].assertions[0].number, 2);
        t.equal(res.tests[1].assertions[0].diag, null);
        ...
        t.equal(res.tests[1].assertions[1].diag, null);
        ...
        t.equal(res.tests[1].assertions[2].diag, null);
        ...
        t.equal(res.tests[1].assertions[3].diag, null);
        ...
        t.equal(res.tests[1].assertions[4].diag, null);
  ...
  t.test("should be able to parse nested test", (t) => {
    const flat = fs.createReadStream(
      path.resolve(__dirname, "./fixtures/nested.txt")
    );
    flat.pipe(
      parser((res) => {
        t.equal(res.ok, true);
        ...
        t.equal(
          res.tests[0].tests[0].name,
          "# should be able to stringify a basic javascript object"
        );
        ...
        t.equal(
          res.tests[0].tests[1].name,
          "# should be able to stringify a complex javascript object"
        );
        ...
        t.equal(
          res.tests[0].tests[2].name,
          "# should be able to parse a complex object"
        );
        ...
        t.equal(
          res.tests[0].tests[3].name,
          "# should be able to stringify a complex javascript object and parse it back"
        );
